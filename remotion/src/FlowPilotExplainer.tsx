...
const VOICEOVER =
  'https://storage.googleapis.com/adm--audio-playback--7d--public/mcp-preview/d10ce02f-fbbc-4227-bbb4-c2a1c2061224.mp3';
...
const Caption: React.FC<{frame: number}> = ({frame}) => {
  const captions = [
    {from: 8, to: 158, text: 'Odpisujesz codziennie na te same pytania?'},
    {from: 166, to: 242, text: 'Wszystkie wiadomości trafiają do jednego inboxu.'},
    {from: 248, to: 356, text: 'AI czyta kontekst i przygotowuje odpowiedź.'},
    {from: 364, to: 482, text: 'Ty sprawdzasz, poprawiasz i zatwierdzasz.'},
    {from: 488, to: 548, text: 'Dopiero wtedy wiadomość trafia do klienta.'},
    {from: 555, to: 760, text: 'Dla usług, e-commerce, salonów, biur i sprzedaży.'},
    {from: 770, to: 888, text: 'Chcesz zobaczyć demo? Napisz: FLOW.'},
  ];
  ...
  return (
    <AbsoluteFill style={{background: C.dark, color: '#fff', opacity: fade(frame, 0, 168)}}>
      ...
      <div style={{position: 'absolute', left: 62, right: 62, top: 830, display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 14, opacity: p2}}>
        {['Messenger', 'Instagram', 'Formularze'].map((label, i) => (
...
const InboxDemo: React.FC<{frame: number}> = ({frame}) => {
  ...
  const draft = 'Jasne. Możemy wdrożyć obsługę Messengera i Instagrama. Najpierw pokażę Ci demo na Twoim procesie.';
    ...
    <AbsoluteFill style={{background: C.bg, color: C.ink, opacity: fade(frame, 150, 558)}}>
      ...
      <div style={{position: 'absolute', left: 46, right: 46, top: 150, bottom: 270, borderRadius: 32, background: '#fff', border: `1px solid ${C.line}`, boxShadow: '0 40px 100px rgba(28,34,48,.14)', overflow: 'hidden', opacity: p, transform: `translateY(${interpolate(p,[0,1],[28,0])}px)`}}>
        <div style={{height: 58, borderBottom: `1px solid ${C.line}`, display: 'flex', alignItems: 'center', padding: '0 18px', gap: 9, background: '#fbfbfc'}}>
          ...
          <div style={{marginLeft: 14, padding: '8px 14px', borderRadius: 10, background: '#f1f2f5', fontSize: 14, color: '#9a9fab'}}>app.flowpilot.ai / inbox</div>
        </div>
        <div style={{display: 'grid', gridTemplateColumns: '235px 1fr 300px', height: 'calc(100% - 58px)'}}>
          ...
          <div style={{padding: 24, position: 'relative'}}>
            ...
            <div style={{position: 'absolute', left: 24, right: 24, bottom: 24, display: 'flex', justifyContent: 'space-between', alignItems: 'center', padding: '14px 16px', borderRadius: 14, border: `1px solid ${C.line}`, background: '#fff'}}>
              <div style={{fontSize: 14, color: C.sub}}>Odpowiedź przygotowana przez AI</div>
          ...
          <div style={{borderLeft: `1px solid ${C.line}`, padding: 20, background: '#fcfcfd', opacity: ai}}>
            <div style={{display: 'flex', alignItems: 'center', justifyContent: 'space-between'}}><div style={{fontSize: 19, fontWeight: 850}}>FlowPilot AI</div><div style={{padding: '7px 10px', borderRadius: 999, background: C.greenSoft, color: C.green, fontSize: 12, fontWeight: 800}}>GOTOWE</div></div>
            <div style={{marginTop: 24, fontSize: 14, color: C.sub, fontWeight: 700}}>AI sprawdziło</div>
            {['kontekst rozmowy', 'ofertę firmy', 'ton komunikacji'].map((x) => <div key={x} style={{marginTop: 11, padding: '12px 13px', borderRadius: 12, background: C.soft, fontSize: 14, fontWeight: 750}}>✓ {x}</div>)}
            <div style={{marginTop: 26, padding: '15px 14px', borderRadius: 13, background: C.blueSoft, color: C.blue, fontSize: 14, lineHeight: 1.35, fontWeight: 750}}>AI przygotowuje draft.<br/>Nie wysyła go bez Ciebie.</div>
          </div>
        </div>
      </div>
      <div style={{position: 'absolute', left: 58, right: 58, bottom: 175, display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10}}>
        {['1. Wiadomość', '2. Kontekst', '3. Draft AI', '4. Zatwierdzenie'].map((x, i) => <div key={x} style={{padding: '15px 10px', borderRadius: 14, textAlign: 'center', background: i <= Math.min(3, Math.floor(local / 100)) ? C.blue : '#fff', color: i <= Math.min(3, Math.floor(local / 100)) ? '#fff' : C.sub, border: `1px solid ${i <= Math.min(3, Math.floor(local / 100)) ? C.blue : C.line}`, fontSize: 15, fontWeight: 820}}>{x}</div>)}
      </div>
    </AbsoluteFill>
  );
};
...
const Audience: React.FC<{frame: number}> = ({frame}) => {
  ...
  const cards = [
    ...
    ['E-commerce', 'pytania przed zakupem i po zamówieniu'],
    ['Salony', 'rezerwacje, ceny, dostępność'],
    ['Biura', 'powtarzalne zapytania od klientów'],
    ['Sprzedaż', 'lead trafia od razu do obsługi'],
  ...
  return (
    <AbsoluteFill style={{background: '#f3f4f7', color: C.ink, opacity: fade(frame, 755, 900)}}>
      ...
      <div style={{position: 'absolute', left: 62, right: 62, top: 350, opacity: p}}>
        <div style={{fontSize: 34, color: C.sub, fontWeight: 760}}>CHCESZ ZOBACZYĆ TO NA SWOJEJ FIRMIE?</div>
        ...
        <div style={{fontSize: 34, lineHeight: 1.2, color: C.sub, marginTop: 34, fontWeight: 700}}>Pokażę Ci dokładnie, jak wygląda<br/>obsługa jednej rozmowy krok po kroku.</div>
...
export const FlowPilotExplainer: React.FC = () => {
  ...
  return (
    <AbsoluteFill style={{fontFamily: 'Inter, ui-sans-serif, -apple-system, BlinkMacSystemFont, Segoe UI, sans-serif'}}>
      ...
      <Sequence from={174} durationInFrames={18}><Audio src={staticFile('audio/notif.wav')} volume={0.42} /></Sequence>
      <Sequence from={462} durationInFrames={12}><Audio src={staticFile('audio/click.wav')} volume={0.45} /></Sequence>
      <Sequence from={500} durationInFrames={20}><Audio src={staticFile('audio/success.wav')} volume={0.38} /></Sequence>
      <Hook frame={frame} />
