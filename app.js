...
let events=[],decisions=new Map(),clients=[],webLeads=[],currentEvent=null;
...
async function api(params,method='GET',mode='json'){
  ...
  if(mode==='text')return res.text();const text=await res.text();try{return JSON.parse(text)}catch{throw new Error('Backend zwrócił nieprawidłową odpowiedź')}
}
function parseEvent(issue){
  try{const raw=JSON.parse(decode64(issue.body));const clientId=decode64(raw.messenger_id_b64)||raw.card?.messenger_id||'';return{issue,eventId:raw.event_id||String(issue.title||'').replace(/^DPCRM\s+/,'').trim(),clientId,mid:decode64(raw.mid_b64),timestamp:raw.timestamp,receivedAt:raw.received_at||'',message:decode64(raw.message_b64),draft:decode64(raw.draft_b64),card:raw.card||{},raw}}catch{return null}
}
function parseDecision(issue){const eventId=String(issue.title||'').replace(/^DPDECISION\s+/,'').trim();const body=String(issue.body||'');const status=(body.match(/(?:^|;)status=([^;]+)/)||[])[1]||'';const draftB64=(body.match(/(?:^|;)draft_b64=([^;]+)/)||[])[1]||'';return eventId?{eventId,status,draft:decode64(draftB64)}:null}
...
function parseWebLead(issue){
  try{const data={};String(issue.body||'').split(';').forEach(part=>{const i=part.indexOf('=');if(i>0)data[part.slice(0,i)]=part.slice(i+1)});return{issue,name:decode64(data.name_b64),email:decode64(data.email_b64),company:decode64(data.company_b64),phone:decode64(data.phone_b64),message:decode64(data.message_b64),source:decode64(data.source_b64),receivedAt:data.received_at||'',id:String(issue.title||'')}}catch{return null}
}
...
function buildClients(){const map=new Map();for(const e of events){const id=e.clientId||e.card?.messenger_id||e.card?.id_klienta||e.eventId;if(!map.has(id))map.set(id,{id,event:e,card:e.card})}clients=[...map.values()]}
function render(){
  buildClients();$('msgCount').textContent=events.length;$('pendingCount').textContent=events.filter(e=>statusOf(e)==='DO AKCEPTACJI').length;$('clientCount').textContent=clients.length+webLeads.length;
  const recentEvents=events.slice(0,4).map(e=>`<div class="listItem" data-event="${esc(e.eventId)}"><b>${esc(displayName(e))}</b><div class="meta">Messenger · ${esc(e.message.slice(0,90)||'Brak treści')} · ${esc(statusOf(e))}</div></div>`);
  const recentWeb=webLeads.slice(0,2).map(l=>`<div class="listItem"><b>${esc(l.name||l.company||l.email||'Lead WWW')}</b><div class="meta">WWW · ${esc(l.message.slice(0,90)||l.email||'Nowe zgłoszenie')}</div></div>`);
  $('recentList').innerHTML=[...recentEvents,...recentWeb].join('')||'<div class="empty">Brak zapisanych wiadomości.</div>';
  $('conversationList').innerHTML=events.length?events.map(e=>`<div class="cv${currentEvent?.eventId===e.eventId?' on':''}" data-event="${esc(e.eventId)}"><b>${esc(displayName(e))}</b><div class="meta">${esc(e.message.slice(0,74)||'Brak treści')}<br>${esc(statusOf(e))}</div></div>`).join(''):'<div class="empty">Brak rozmów Messenger.</div>';
  $('inboxMeta').textContent=`${events.length} wiadomości · ${events.filter(e=>statusOf(e)==='DO AKCEPTACJI').length} do akceptacji`;
  const messengerRows=clients.map(x=>{const c=x.card,e=x.event;return `<tr><td><b>${esc(displayName(e))}</b><div class="meta">Messenger · ${esc(maskId(e.clientId))}</div></td><td>${esc(locationOf(c))}</td><td>${esc(money(investmentOf(c)))}</td><td>${esc(val(salesStatus(c,e)))}</td><td>${esc(val(nextStep(c)))}</td></tr>`});
  const webRows=webLeads.map(l=>`<tr><td><b>${esc(l.name||l.company||'Lead WWW')}</b><div class="meta">WWW · ${esc(l.email||l.phone||'brak kontaktu')}</div></td><td>${esc(l.company||'brak danych')}</td><td>brak danych</td><td>Nowy lead WWW</td><td>${esc(l.message||'Skontaktować się')}</td></tr>`);
  $('crmRows').innerHTML=[...messengerRows,...webRows].join('')||'<tr><td colspan="5" class="empty">Brak kart klientów.</td></tr>';
  document.querySelectorAll('[data-event]').forEach(el=>el.onclick=()=>selectEvent(el.dataset.event));if(currentEvent)showEvent(currentEvent);else if(events.length)selectEvent((events.find(e=>statusOf(e)==='DO AKCEPTACJI')||events[0]).eventId);else clearEditor()
}
...
function showEvent(e){
  ...
  const c=e.card||{},preferred=['status_sprzedazy','status_realizacji','status_formularza','status_platnosci','status_researchu_finansowania','rekomendowany_instrument_finansowania','lokalizacja','miasto','branza','branża','wartosc_inwestycji','budzet','mozliwa_wartosc_finansowania','poziom_ryzyka','nastepny_krok','termin_nastepnego_dzialania'];const entries=[];for(const k of preferred){if(k in c&&!entries.some(([x])=>x===k))entries.push([k,c[k]])}for(const [k,v] of Object.entries(c)){if(entries.length>=10)break;if(!entries.some(([x])=>x===k)&&!['ostatnia_wiadomosc_klienta','ostatni_szkic_ai','messenger_id'].includes(k))entries.push([k,v])}
  $('cardDetails').innerHTML=entries.length?entries.map(([k,v])=>`<div class="detail"><small>${esc(humanize(k))}</small><b>${esc(val(v))}</b></div>`).join(''):'<div class="detail"><small>Karta CRM</small><b>Brak dodatkowych danych.</b></div>';$('actionState').textContent='';$('actionState').className='actionState'
}
async function loadProduction(newKey){
  $('authError').textContent='';if(newKey)sessionStorage.setItem('flowpilot_operator_key',newKey);if(!key())throw new Error('Wpisz klucz operatora');setConnected(false,'łączenie');$('healthState').textContent='...';
  const [rawEvents,rawDecisions,rawWeb]=await Promise.all([api({action:'panel'}),api({action:'decisions'}),api({action:'web'})]);decisions=buildDecisionMap(Array.isArray(rawDecisions)?rawDecisions:[]);events=(Array.isArray(rawEvents)?rawEvents:[]).map(parseEvent).filter(Boolean).sort((a,b)=>String(b.receivedAt).localeCompare(String(a.receivedAt)));webLeads=(Array.isArray(rawWeb)?rawWeb:[]).map(parseWebLead).filter(Boolean).sort((a,b)=>String(b.receivedAt).localeCompare(String(a.receivedAt)));
  setConnected(true,`${events.length} rekordów CRM · ${webLeads.length} leadów WWW`);$('auth').classList.remove('on');render();return true
}
...
async function sendApproved(){
  if(!currentEvent||!currentEvent.clientId)return;const state=$('actionState'),plain=$('reply').value.trim();if(!plain){state.textContent='Szkic jest pusty.';state.className='actionState error';return}if(!confirm('Wysłać tę wiadomość do klienta na Messengerze?'))return;state.textContent='Wysyłanie do Messengera...';state.className='actionState';$('send').disabled=true;
  try{const result=await api({action:'send',event_id:currentEvent.eventId,client_id:currentEvent.clientId,message:currentEvent.message,draft:jsonSafeForMake(plain)},'POST');decisions.set(currentEvent.eventId,{eventId:currentEvent.eventId,status:'ZATWIERDZONO',draft:plain});state.textContent=result.decision_logged===false?'Wysłano do klienta. Zapis decyzji w CRM wymaga kontroli.':'Wysłano do klienta i zapisano decyzję.';if(result.decision_logged===false)state.className='actionState error';render();toast('Wiadomość wysłana')}catch(e){state.textContent='Nie wysłano: '+e.message;state.className='actionState error';$('send').disabled=false}
}
document.querySelectorAll('.nav button').forEach(b=>b.onclick=()=>go(b.dataset.v));$('connect').onclick=showAuth;$('refresh').onclick=()=>loadProduction().catch(e=>{setConnected(false);toast(e.message)});$('authCancel').onclick=()=>$('auth').classList.remove('on');$('authConnect').onclick=async()=>{try{await loadProduction($('operatorKey').value.trim())}catch(e){sessionStorage.removeItem('flowpilot_operator_key');$('authError').textContent=e.message;setConnected(false)}};$('operatorKey').onkeydown=e=>{if(e.key==='Enter')$('authConnect').click()};$('regenerate').onclick=regenerate;$('reject').onclick=reject;$('send').onclick=sendApproved;if(key())loadProduction().catch(()=>{sessionStorage.removeItem('flowpilot_operator_key');setConnected(false)});else setConnected(false);
